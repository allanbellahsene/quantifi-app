...
const RollingSharpeChart = ({ data, strategies, assetName, startDate, endDate }) => {
    ...
    <Card elevation={3} sx={{ marginTop: 4, marginBottom: 4 }}>
      <CardContent>
        <Typography variant="h5" gutterBottom sx={{ fontWeight: 'bold', color: theme.palette.primary.main }}>
          {`Rolling Sharpe Ratio - ${assetName}`}
        </Typography>
            ...
            {strategies.map((strategy, index) => 
              visibleStrategies[strategy.name] && (
                <Line
                  key={strategy.name}
                  type="monotone"
                  dataKey={`${strategy.name}_rolling_sharpe`}
                  name={strategy.name}
