...
def create_indicator(df: pd.DataFrame, indicator: str, window: int = None) -> pd.Series:
    try:
        ...
        backtest_logger.error(f"Error creating indicator: {str(e)}")
        raise
...
def create_condition(df: pd.DataFrame, rule: Rule) -> pd.Series:
    try:
        ...
        backtest_logger.error(f"Error creating condition: {str(e)}")
        raise
...
def handle_special_floats(value):
    if np.isnan(value):
        ...
        return None  # or you could return a very large number like 1e300
    else:
        return value
...
def run_backtest(symbol: str, start_date: str, end_date: str, strategy: Strategy):
    try:
        ...
        # Calculate metrics
        total_return = handle_special_floats(df['Cumulative_Returns'].iloc[-1] - 1)
        sharpe_ratio = handle_special_floats(np.sqrt(252) * df['Strategy_Returns'].mean() / df['Strategy_Returns'].std())
        max_drawdown = handle_special_floats((df['Cumulative_Returns'] / df['Cumulative_Returns'].cummax() - 1).min())
        ...
        return {
            ...
            "equity_curve": [handle_special_floats(x) for x in df['Cumulative_Returns'].tolist()],
            "dates": df.index.strftime('%Y-%m-%d').tolist()
        }
    except Exception as e:
        backtest_logger.error(f"Error in run_backtest: {str(e)}")
        backtest_logger.error(traceback.format_exc())
        raise
